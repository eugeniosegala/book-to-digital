...
### Requirement: vision-llm module
**Reason**: `callVisionOpenRouter` is now part of the `openrouter-client` module; the standalone `vision-llm.ts` file is deleted.
**Migration**: Import `callVisionOpenRouter` from `../clients/openrouter.js` instead of `../clients/vision-llm.js`.
