...
vi.mock('@aws-sdk/client-textract', () => {
  ...
  const MockTextractClient = vi.fn().mockImplementation(() => ({ send: mockSend }));
  const AnalyzeDocumentCommand = vi.fn().mockImplementation((input: unknown) => input);
  return { TextractClient: MockTextractClient, AnalyzeDocumentCommand, mockSend };
});
