...
describe('translatePages', () => {
  ...
  it('handles LLM failure gracefully — keeps original text', async () => {
    const errorResponse = { ok: false, status: 500, text: async () => 'server error' };
    mockFetch
      .mockResolvedValueOnce(okResponse(['Title EN']))
  ...
  it('retries a block when a source-language span remains in the translation', async () => {
    const sourceText = 'Die Stadt war damals sehr reich und überaus mächtig in der ganzen Region bekannt.';
    const badTranslation = 'The city was damals sehr reich und überaus mächtig in the whole region.';
    const goodTranslation = 'The city was at that time very rich and exceedingly powerful, known throughout the entire region.';
    ...
    expect(retryBody.messages[0].content).toContain('A previous attempt left source-language wording behind');
  });

  it('retries only the block with untranslated text in a multi-block page', async () => {
    const source1 = 'Der Künstler wurde im Jahre vierzehnhundertsiebzig in der kleinen Stadt geboren.';
    const source2 = 'Seine Werke sind heute noch in vielen bedeutenden europäischen Museen ausgestellt und bewundert.';
    const good1 = 'The artist was born in the year fourteen seventy in the small town.';
    const bad2 = 'His works are heute noch in vielen bedeutenden europäischen Museen exhibited and admired.';
    const good2 = 'His works are still exhibited and admired in many important European museums today.';
  ...
  it('retries when untranslated span appears at the start of translation', async () => {
    const source = 'Die berühmte gotische Kathedrale wurde im dreizehnten Jahrhundert erbaut und später erweitert.';
    const bad = 'Die berühmte gotische Kathedrale wurde built in the thirteenth century and later expanded.';
    const good = 'The famous Gothic cathedral was built in the thirteenth century and later expanded.';
  ...
  it('retries when untranslated span appears at the end of translation', async () => {
    const source = 'The monastery was founded by monks who wanderten durch die weiten unbekannten Gebiete des Landes.';
    const bad = 'The monastery was founded by monks who wanderten durch die weiten unbekannten Gebiete des Landes.';
    const good = 'The monastery was founded by monks who wandered through the vast unknown territories of the land.';
  ...
  it('keeps retry result even if retry still contains untranslated text', async () => {
    const source = 'Das Kloster wurde von den wandernden Mönchen gegründet die durch das weite unbekannte Land zogen.';
    const bad = 'The monastery was von den wandernden Mönchen gegründet die durch das weite unbekannte Land founded.';
    const stillBad = 'The monastery was founded by the wandernden Mönchen who travelled through the vast unknown land.';
  ...
  it('does not retry when source is non-Latin short text', async () => {
    ...
    const pages = [
      makePage(1, [{ type: BlockType.TEXT, text: 'СЕШ C E E' }]),
    ];
  ...
  it('does not retry when untranslated span is mostly proper nouns', async () => {
    ...
    const translation = 'Ingeniosus Magister Der Augsburger Bildhauer Gregor Erhart';

    mockFetch.mockResolvedValueOnce(okResponse([translation]));

    const pages = [
      makePage(1, [{ type: BlockType.TEXT, text: source }]),
    ];
  ...
  it('does not retry when lowercase words in span are fewer than 3', async () => {
    ...
    const source = 'Kaiser Friedrich der Große regierte Preußen mit eiserner Hand';
    const translation = 'Kaiser Friedrich der Große ruled Prussia with an iron hand';
  ...
  it('sends correct model and structured output format', async () => {
    ...
    await translatePages([makePage(1, [{ type: BlockType.TEXT, text: 'DE' }])], opts);
  ...
  it('handles hyphen at end of multi-word block', async () => {
    ...
    const pages = [
      makePage(1, [{ type: BlockType.TEXT, text: 'Er wohnte im Zisterzienser-' }]),
