...
describe("reorderBlocks", () => {
  ...
  it("returns original order when LLM returns wrong number of indices", async () => {
    ...
    mockFetch.mockResolvedValueOnce(okResponse(JSON.stringify({ order: [1] })));
  ...
  it("snaps caption to closest figure when multiple figures exist", async () => {
    ...
    expect(result[1].boundingBox.top).toBe(0.6);
  ...
  it("removes TEXT block that duplicates the preceding FIGURE_CAPTION", async () => {
    ...
    expect(
      result.find(
        (b) => b.text.startsWith("Abb. 2") && b.type === BlockType.TEXT,
      ),
    ).toBeUndefined();
    ...
    expect(
      result.find(
        (b) =>
          ...
          b.text.startsWith("Abb. 3. Allianzwappen"),
      ),
    ).toBeUndefined();
  ...
  it("removes a trailing caption fragment repeated as a nearby TEXT block", async () => {
    ...
    expect(
      result.find(
        (b) => b.type === BlockType.TEXT && b.text === "Murau, St. Matthäus",
      ),
    ).toBeUndefined();
    expect(result[2].text).toBe("Weiterer Fließtext beginnt hier.");
  });

  it("merges adjacent TEXT blocks when a paragraph wraps into the next column", async () => {
    const blocks = [
      makeBlock(
        BlockType.TEXT,
  ...
  it("does not merge cross-column TEXT blocks when the previous one ends a sentence", async () => {
    const blocks = [
      makeBlock(
        BlockType.TEXT,
